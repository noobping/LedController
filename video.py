...
def send_packet(ip: str, port: int, packet: bytes) -> None:
    """
    Sends a UDP packet to a specified WLED controller.
    Args:
    ...
    """
    try:
        # Create a new UDP socket for each packet
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                # Extract colors for each window (flattened to a list of pixels)
...
def play_playlist(playlist_path: str, loop: bool = False, max_fps: float = None) -> None:
    ...
    while True:
        for video_file in videos:
            logging.info(f"Starting video playback: {video_file}")
            play_video(video_file, loop=False, max_fps=max_fps)
