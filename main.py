...
def reverse_state():
    ...
    for row in range(rows):
        tempState.extend(
            currentState[row * ledsInRow: (row+1) * ledsInRow][::-1])
    return tempState
    ...
    try:
        while True:
            ...
            match data[0]:
                ...
                case b"difference":
                    differences = [str(diff.strip(b"()"), encoding='utf-8')
                                   for diff in data[1].split(b", ")]
...
def video_playback(videoName: str):
    ...
    while True:
        ...
        frameState = [''.join([f"{rgb:02x}" for rgb in pixel])
                      for pixel in frame.reshape(-1, frame.shape[-1])]
        frameCount += 1
